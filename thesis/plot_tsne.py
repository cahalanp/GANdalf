...
if data_flag:
    ...
    print("xyz_r_std.shape",np.array(xyz_r_std).shape)    
    print("xyz_g_std.shape",np.array(xyz_g_std).shape)
    
    tsne = TSNE(n_components=2, init='pca', random_state=90, angle = 0.3, perplexity=5)
    x = np.concatenate((xyz_r_std, xyz_g_std),0)
    x = tsne.fit_transform(x)
    
    xyz_r_std_pca = x[:len(mols_r)]
    xyz_g_std_pca = x[len(mols_r):]
    #xyz_r_std_pca = tsne.fit_transform(xyz_r_std)
    #xyz_g_std_pca = tsne.fit_transform(xyz_g_std)
...
if plot_flag:
    ...
    fig.text(0.98, 0.5, 'DFT Energy [eV/atom]', ha='center', va='center', rotation='vertical', fontsize=16)
