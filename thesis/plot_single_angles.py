...
Version:
    ...
    - Invert samples
    - Make plot_angles with inverted samples
"""
...
if plot_flag:
    ...
    for i in [actual_mol]:
        ...
        ax.text(0.5, 0.85, r"\underline{%s}" % names2[i], ha='center', va='center', transform=ax.transAxes, fontsize=15)
        ...
        #ax.hist(data[i][0], density=density, bins=bins0, histtype='step', color=colors[0], label=data_labels[0], lw=1)
        #ax.hist(data[i][1], density=density, bins=bins1, histtype='step', color=colors[1], label=data_labels[1], lw=1)
        
        dummy_x = np.arange(180)
        _, _, poly_r = ax.hist(dummy_x, density=density, bins=range(0,180), histtype='step', color=colors[0], label=data_labels[0], lw=1)
        _, _, poly_g = ax.hist(dummy_x, density=density, bins=range(0,180), histtype='step', color=colors[1], label=data_labels[1], lw=1)
        ...
        ax.set_ylim(0,max(data[i][1])*1.2)

    # Plot Settings
    #fig.supylabel('Count', fontsize=8)
    fig.text(0.045, 0.5, 'Frequency [arb. unit]', ha='center', va='center', rotation='vertical', fontsize=17)
    ax.set_xlabel(r'$\mathrm{\theta}_{ijk}$ [deg]', fontsize=17)
    ...
    plt.savefig('./thesis/plot_all_angles/plots/all_angles2_'+str(time_date)+'.png', dpi=500, bbox_inches='tight')
    #plt.show()
