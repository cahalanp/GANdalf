...
if plot_flag:
    ...
    # fake the data from James thesis
    cc = np.concatenate((np.random.normal(1.39, 0.017, 5600), np.random.normal(2.405, 0.025, 6500), np.random.normal(2.78, 0.025, 3000)))
    #cc = np.expand_dims(cc, axis=0)
    ch = np.concatenate((np.random.normal(1.065, 0.017, 7000), np.random.normal(2.13, 0.025, 13000), np.random.normal(3.372, 0.034, 14000), np.random.normal(3.85, 0.032, 6000)))
    #ch = np.expand_dims(ch, axis=0)
    hh = np.concatenate((np.random.normal(2.47, 0.035, 10000), np.random.normal(4.25, 0.046, 11000), np.random.normal(4.915, 0.04, 4000)))
    ...
    for i in [0, 1, 2]:
        ...
        axs[i].text(0.5, 0.85, r"\underline{%s}" % names_ax[i], ha='center', va='center', transform=axs[i].transAxes, fontsize=19)
    ...
    axs[2].set_xlabel('$r_{ij}$ [\AA]', fontsize=19)
    #axs[0].set_xticks([])
    #axs[0].set_yticks([])
