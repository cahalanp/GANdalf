...
if data_flag:
    ...
    if not temp:
        ...
        invert_path = "/home/cahalanp/phd/gandalf/james/1SingleMode/SavedNet/12_benzene/{}/plots2/".format(s_G)
        #invert_path = "/home/cahalanp/phd/gandalf/james/1SingleMode/SavedNet/12_benzene/0132_0911/plots2/"
        
        fls = [f for f in os.listdir(invert_path)]
        for f in fls:
            print(f)
            if f[-4:] != ".xyz":
                os.remove(invert_path+f)
        fls = sorted([f for f in os.listdir(invert_path) if f[-4:] == ".xyz"])
        print(invert_path)
        print(fls)
        xyz_g, l_g = utils.read_xyz_dir(N_max_r, invert_path, sorted_flag=1)
        ...
        if nn_g:
            ...
            z = G2.make_seeds(100000, gs['seeds'], 'normal').double() 
            ...
            print(xyzs.shape)
            xyz_g = xyzs
            l_g = [l_g[0] for _ in range(len(xyzs))]
