...
def gradient_penalty(
    ...
    real_e
        Conditioning energies for the batch.
    fake_b
    ...
    interpolated_e = epsilon[0,:,0].unsqueeze(1)
    
    # Compute discriminator score for interpolated batch
    interpolated_score = nn_D(interpolated_b, real_l, interpolated_e) # fake_l must be same as real_l ???
...
class Discriminator(nn.Module):
    """
    GAN discriminator for assessing molecular features conditioned on atomic labels and energies.
    ...
    def __init__(self, info: dict, GS: dict):
        ...
        if self.depth == 0:
            self.fc_d1 = nn.Linear(self.N_species*self.N_feats + self.N_units_l + self.N_E, self.N_units, bias=True)
            self.fc_d2 = nn.Linear(self.N_units, 1, bias=True)
            
        elif self.depth == 1:
            self.fc_d1 = nn.Linear(self.N_species*self.N_feats + self.N_units_l + self.N_E, 2*self.N_units, bias=True)
            self.fc_d2 = nn.Linear(2*self.N_units, self.N_units, bias=True)
            self.fc_d3 = nn.Linear(self.N_units, 1, bias=True)
            
        elif self.depth == 2:
            self.fc_d1 = nn.Linear(self.N_species*self.N_feats + self.N_units_l + self.N_E, 4*self.N_units, bias=True)
    ...
    def forward(self, b: torch.Tensor, l: torch.Tensor, e: torch.Tensor) -> torch.Tensor:
        """
        ...
        e
            Molecular energies tensor of shape (N_batch,)
        ...
        for i_set, atomic_label in enumerate(self.atomic_labels): 
            atomic_label = torch.tensor(atomic_label, dtype=torch.int32).to(self.device)
            if len(atomic_label) == N_at:
                if torch.all(l == atomic_label):
                    l_idx = torch.tensor(i_set, dtype=torch.int32) 
                    break 
        else:
        ...
        e = e.repeat(1,self.N_E) # [N_batch] -> [N_batch,N_e]
        ...
        if print_flag:
            ...
            print("l.shape",l.shape)
            print("e.shape",e.shape)
            print("cat((b,l,e), 1).shape",torch.cat((b,l,e), 1).shape)
        
        # Pass [b,l,e] through FC layers
        x = torch.cat((b,l,e), 1)
