...
def make_seeds(
    ...
    """
    Generate latent seeds, atomic labels, and corresponding energy samples for GAN training or inference.
    ...
    e
        Tensor of sampled energies reshaped to `(N_batch, 1)`, used as conditioning inputs.
    """
    ...
    # sample fake energies using kde
    e = torch.tensor(info['kdes'][set_ind].resample(N_batch).reshape((-1,1))).to(GS['device']).type(dtype=z.type())
...
class Generator(nn.Module):
    """
    GAN generator for producing molecular features conditioned on atomic labels and energies.
    ...
    def __init__(self, info: dict, GS: dict):
        ...
        if self.l_fc:
            ...
            self.fc_g1 = nn.Linear(self.N_z + self.N_units_l + self.N_E, self.N_units, bias=True)
        else:
            self.fc_g1 = nn.Linear(self.N_z + self.N_set + self.N_E, self.N_units, bias=True)
    ...
    def forward(self, z: torch.Tensor, l: torch.Tensor, e: torch.Tensor) -> torch.Tensor:
        """
        ...
        if print_flag:
            ...
            print("l.shape",l.shape)
            print("e.shape",e.shape)
            print("cat((z,l,e), 1).shape",torch.cat((z,l,e), 1).shape)
